#!/usr/bin/env python3
"""
Generate an interactive dependency graph HTML from dependency-graph.json.
...
Usage:
    python generate_dependency_graph.py --analysis-dir /path/to/analysis/output [--project-name MyProject]
"""
...
CATEGORY_META = {
    ...
    "python":       {"label": "Python module",     "color": "#3572A5"},
    "other":        {"label": "other",             "color": "#6e7681"},
}


def categorise(name: str) -> str:
    """Categorise a node by its file path or module name."""
    ...
    if name.startswith("extensions/") or name.startswith("plugins/"):
        return "extension"
    if name.startswith("ui/") or name.startswith("ui-src/") or name.startswith("frontend/"):
        return "ui"
    if name.startswith("scripts/") or name.startswith("test/") or name.startswith("tests/"):
        return "scripts"
    if name.endswith(".py"):
        return "python"
    # Heuristic: if it has no path separators and no file extension → npm/pip package
    has_ext = any(name.endswith(e) for e in (
        ".ts", ".tsx", ".js", ".mjs", ".cjs", ".json", ".py", ".rb",
        ".java", ".kt", ".swift", ".go", ".rs", ".cpp", ".c", ".h",
    ))
    if not has_ext and "/" not in name:
        return "npm-package"
...
@click.command()
@click.option("--analysis-dir", required=True, type=click.Path(exists=True),
              help="Path to the analysis output directory (contains dependencies/ subfolder)")
@click.option("--project-name", default=None,
              help="Project name for the page title (auto-detected from directory if omitted)")
def main(analysis_dir: str, project_name: str | None):
    """Generate an interactive dependency graph HTML from analysis data."""
    analysis_path = Path(analysis_dir)
    deps_dir = analysis_path / "dependencies"

    graph_json_path = deps_dir / "dependency-graph.json"
    circular_json_path = deps_dir / "circular-dependencies.json"
    output_html_path = deps_dir / "dependency-graph.html"

    if not graph_json_path.exists():
        raise click.ClickException(f"dependency-graph.json not found in {deps_dir}")

    if project_name is None:
        # Walk up to find the project name from the directory structure
        # Expected: .../{project-name}/{timestamp}/dependencies/
        project_name = analysis_path.name
        if project_name.isdigit() or (len(project_name) >= 14 and project_name[:8].isdigit()):
            # Looks like a timestamp – use the parent
            project_name = analysis_path.parent.name
    ...
    try:
        with open(circular_json_path, encoding="utf-8") as fh:
    ...
    for name, nid in node_ids.items():
        ...
        nodes.append({
            ...
            "y": r * math.sin(angle),
            "size": round(size, 2),
            "color": CATEGORY_META.get(cat, CATEGORY_META["other"])["color"],
            "category": cat,
    ...
    for name, info in raw.items():
        detail[str(node_ids[name])] = {
            ...
            "functions_used":      list(info.get("functions_used",      {}).keys()) if isinstance(info.get("functions_used"), dict) else [],
            "functions_providing": list(info.get("functions_providing", {}).keys()) if isinstance(info.get("functions_providing"), dict) else [],
            "classes_used":        list(info.get("classes_used",        {}).keys()) if isinstance(info.get("classes_used"), dict) else [],
            "classes_providing":   list(info.get("classes_providing",   {}).keys()) if isinstance(info.get("classes_providing"), dict) else [],
        }
    ...
    title_escaped = project_name.replace('"', '&quot;').replace('<', '&lt;')
    ...
    html = _build_html(
        ...
        cat_meta_json=cat_meta_json,
        circular_nodes_json=json.dumps(sorted(circular_nodes)),
    )

    output_html_path.write_text(html, encoding="utf-8")
    size_mb = output_html_path.stat().st_size / 1024 / 1024
    print(f"Done! → {output_html_path}")
    print(f"  File size: {size_mb:.1f} MB")
    print()
    print("Open the HTML file in a browser to explore the graph:")
    print(f"  open {output_html_path}")
...
def _build_html(
    *,
    ...
    detail_json: str,
    cat_meta_json: str,
    circular_nodes_json: str,
) -> str:
    """Return a self-contained HTML string for the interactive dependency graph."""
...
const CIRCULAR   = new Set({circular_nodes_json});
</script>
...
function showInspector(nodeId) {{
  ...
  c.innerHTML = `
    ...
    <div class="insp-section">
      <h4>Functions used (${{(d.functions_used||[]).length}})</h4>
